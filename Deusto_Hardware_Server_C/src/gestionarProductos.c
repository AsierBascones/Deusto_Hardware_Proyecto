...
void eliminarLineaCarritoProducto(sqlite3 *db, int idProducto) {
	char sql1[] =
			"select L.id_carrito, L.preio_unitrio, P.total from PEDIO P, LINEA_CARRITO L where L.id_carrito = P.id_carrito and L.id_producto = ?";
...
void anyadirProductos(sqlite3 *db) {
	...
	char sql3[] =
			"insert or ignore into PRODUCTO (nombre, descripcion, precio, stock, marca, id_categoria, id_proveedor) values (?, ?, ?, ?, ?, ?, ?)";
...
void getAndPrintProducto(sqlite3 *db, int idProd) {
	...
	sqlite3_finalize(stmt);
	printProductoCompleto(idProdTemp, nomProd, descripcion, marca, precio,
			stock, nomCat, nomProv);
}
...
void getLineaProd(sqlite3 *db, int idProd, double precio, double precioAntiguo) {
	...
	sqlite3_stmt *stmt;
	char sql1[] =
			"select P.id_carrito, P.total, L.cantidad from LINEA_CARRITO L, PEDIDO P where L.id_producto = ? and P.id_carrito = L.id_carrito";
	sqlite3_prepare_v2(db, sql1, strlen(sql1), &stmt, NULL);
	sqlite3_bind_int(stmt, 1, idProd);
	do {
		result = sqlite3_step(stmt);
		if (result == SQLITE_ROW) {
			...
			if (idCarAntiguo != idCar) {
				actualizarLineaProd(db, idProd, idCar, precio * cantidad,
						total);
			}
		}
	} while (result == SQLITE_ROW);
...
void actualizarLineaProd(sqlite3 *db, int idProd, int idCar, double precio,
		double total) {
	int result;
	sqlite3_stmt *stmt;
	char sql[] =
