...
void eliminarLineaCarritoProducto(sqlite3 *db, int idProducto) {
    char sql1[] = "select L.id_carrito, L.precio_unitario, P.total from PEDIDO P, LINEA_CARRITO L where L.id_carrito = P.id_carrito and L.id_producto = ?";
    ...
    // Solo actualizamos si hemos encontrado carritos válidos
    if (idCar != 0) {
...
void modificarProductos(sqlite3 *db) {
    ...
    while (permanecer) {
        ...
        printf("1. Stock de un producto\n");
        printf("2. Precio de un producto\n");
        printf("3. Nombre de un producto\n");
        printf("4. Descripcion de un producto\n");
        printf("5. Marca de un producto\n");
        printf("6. Categoria de un Producto\n");
        printf("7. Proveedor del Producto\n");
        fflush(stdout);
...
void actualizarLineaProd(sqlite3 *db, int idProd, int idCar, double precio, double total) {
    ...
    char sql[] = "update LINEA_CARRITO set precio_unitario = ? where id_producto = ? and id_carrito = ?";
    sqlite3_prepare_v2(db, sql, strlen(sql), &stmt, NULL);
